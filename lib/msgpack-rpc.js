...
EncodeStream.prototype._transform = function (data, encoding, callback) {
    const buffer = msgpack.encode(data, { codec: msgpack.createCodec({ useraw: true }) });
    this.push(buffer);
    callback();
};
...
function createEncodeStream(options = {}) {
    return EncodeStream(); // msgpacklite.createEncodeStream({ codec: msgpack.createCodec({ useraw: true }) });
}
...
function Client(socket) {
    ...
    const self = this,
        ...
        encodeStream = createEncodeStream(),
        send = function send(request) {
            encodeStream.write(request, () => {
    ...
    socket.pipe(msgpack.createDecodeStream()).on('data', receive);
