...
EncodeStream.prototype._transform = function (data, encoding, callback) {
    const buffer = msgpackjs.encode(data);
    this.push(buffer);
    callback();
};
...
function createEncodeStream(options) {
    return EncodeStream(); // msgpacklite.createEncodeStream(options);
}
...
function Client(socket) {
    ...
    socket.pipe(msgpacklite.createDecodeStream()).on('data', receive);
...
exports.createServer = function createServer(options, connectionListener) {
    ...
    server.on('connection', function onConnection(socket) {
        ...
        encodeStream.pipe(socket);
        socket.pipe(msgpacklite.createDecodeStream()).on('data', request => {
            debug(request);
            if (request[0] === 0) {
