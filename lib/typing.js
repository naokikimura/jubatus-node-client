...
function handleCasting(handlers, value, schema, types) {
    const { responsible, executor } = handlers[0] || { responsible: () => true, executor: (value) => value };
    return responsible(schema) ? executor(value, schema, types) : handleCasting(handlers.slice(1), value, schema, types);
}
...
function createTypes(definitions, commonTypes) {
    const types = _.toPairs(definitions)
...
function defineFromTupleFunction(types, definitions, commonTypes) {
    ...
    return _.toPairs(types)
        .map(([typeName, constructor]) => {
