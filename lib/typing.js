...
function toCamelCase(string) {
    return _.replace(string, /(_)([a-z])/g, (match, g1, g2) => g2.toUpperCase());
}
...
function toSnakeCase(string) {
    return _.replace(string, /((^[A-Z])|([A-Z]))/g, (m, g1, g2, g3) => g3 ? '_' + g3.toLowerCase() : g2.toLowerCase());
}
...
function createType(typeName, schema) {
    ...
    constructor.prototype.toTuple = function () {
        return _.at(this, keys).filter(value => !_.isUndefined(value)).map(toTuple);
    };
    return constructor;
}
